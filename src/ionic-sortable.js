;(function(window, angular){
	...
	module.directive('svRoot', [function(){
		function shouldBeAfter(elem, pointer, isGrid, rect){
			if(isGrid) {
				var isInArea = (rect.left < pointer.x && pointer.x < rect.right) && (rect.top < pointer.y && pointer.y < rect.bottom);
				return !isInArea;
			} else {
		...
		return {
			...
			controller: ['$scope', '$attrs', '$interpolate', '$parse', function($scope, $attrs, $interpolate, $parse){
				...
				this.$moveUpdate = function(opts, mouse, svElement, svOriginal, svPlaceholder, originatingPart, originatingIndex){
					...
					var body = angular.element(document.querySelector('.organizePagesContent'))[0];
					...
					getSortableElements(mapKey).forEach(function(se, index){
						...
						if(!se.container && // not the container element
							...
							candidates.push({
								...
								q: Math.sqrt( Math.pow((center.x-mouse.x), 2) + Math.pow((center.y-mouse.y), 2) ),
								view: se.getPart(),
								targetIndex: se.getIndex(),
								after: shouldBeAfter(center, mouse, isGrid, rect)
							});
						}
					...
					candidates.push({
						...
						q: Math.sqrt( Math.pow((pCenter.x-mouse.x), 2) + Math.pow((pCenter.y-mouse.y), 2) ),
						element: $placeholder,
						placeholder: true
					});
				...
				this.$drop = function(originatingPart, index, options){
					...
					if(options.revert){
						...
						var bodyRect = body.getBoundingClientRect();
						$helper.css({
							'top': placeholderRect.top + body.scrollTop - bodyRect.top+ 'px',
	...
	module.directive('svElement', ['$parse', function($parse){
		return {
			...
			link: function($scope, $element, $attrs, $controllers){
				...
				var touchY, pageHeight, containerTop, maxScroll, viewHeight;
				var scrollBorder = 20, scrollSpeed = 0.05;
				var sortableElement = {
					element: $element,
				...
				handle.on('mousedown touchstart', function (e) {
					...
					var bodyScrollTopStart = body.scrollTop;
					setTimeout(function () {
						if ($scope.longPress) {
							var bodyScrollTopEnd = body.scrollTop;
							var scrollDiff = Math.abs(bodyScrollTopStart - bodyScrollTopEnd);
							var handleRect = handle[0].getBoundingClientRect();
							if (scrollDiff < (handleRect.height/2)) {
								onMousedown(e);
							}
						}
					}, 300);
				...
				function initAutoScroll(e) {
					touchY = e.clientY;
					var scrollArea = closestElement.call($element, '.scroll');
					var scrollAreaRect = scrollArea[0].getBoundingClientRect();
					var viewContainer = scrollArea.parent();
					var viewContainerRect = viewContainer[0].getBoundingClientRect();
					viewHeight = viewContainerRect.height;
					containerTop = viewContainerRect.top;
					var pagesContainer = angular.element(document.querySelector('.pages-container'));
					var pageContainerRect = pagesContainer[0].getBoundingClientRect();
					pageHeight = pagesContainer.height;
					maxScroll = pageHeight - scrollAreaRect.height;
				...
				function autoScroll() {
					...
					} else if (touchY >= 0 && touchY > viewHeight - scrollBorder) {
						// Should scroll down
						scrollChange = touchY - (viewHeight - scrollBorder);
					}

					if (scrollChange !== 0) {
						// get the updated scroll position
						var newScroll = body.scrollTop + scrollSpeed * scrollChange;
						// Apply scroll limits
						if (newScroll < 0)
							newScroll = 0;
				...
				function onMousedown(e) {
					...
					clone[0].reposition = function (coords) {
						...
						if (containmentRect) {
							if (targetTop < containmentRect.top){}
							else if (targetTop < containmentRect.top + body.scrollTop) { // top boundary
								targetTop = containmentRect.top + body.scrollTop;
							} else if (targetTop + helperRect.height > containmentRect.top + body.scrollTop + containmentRect.height) {
					...
					var pointerOffset = {
						...
						y: (e.clientY - clientRect.top + bodyRect.top) / clientRect.height
					};
	...
	function insertElementBefore(element, newElement){
		...
		if(prevSibl.length > 0){
			if(prevSibl[0].className == "sv-placeholder") {
				element.parent().after(newElement);
			}else {
